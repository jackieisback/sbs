...
path_to_irfanview = r'"C:\Program Files\IrfanView\i_view64.exe" '
...
def moveAndConvertImages():
  ...
  for idx, file in enumerate(rel_files):
    ...
    if os.path.getctime(fullpath) > cr_t:
      ...
      commandstring = path_to_irfanview + filestring + r' /resize_long=350 /jpgq=90 /aspectratio /convert=' + pic_dir + "\\" + name + "\""
...
def editContent(contents, lineNumber, totalLines):
        if "Min." in contents:
          ...
          #addtimes(contents) TODO
          return 1
        elif "Kein Zeitunterschied." in contents:
          editor.replaceLine(lineNumber, "<b>" + contents.rstrip("\r\n") + "</b>")
          #addtimes(contents) TODO
          return 1
        elif "[00:" in contents:
